...
def procesar_datos_jerarquicos(df, hacer_pivot=True, timeout_minutos=5):
    """
    Procesa datos jerárquicos con totalizaciones y formato simplificado.
    Opcionalmente, realiza una transformación pivot por Compo_Coste.
    
    Args:
        df (DataFrame): DataFrame con los datos jerárquicos
        hacer_pivot (bool): Si es True, realiza transformación pivot por Compo_Coste
        timeout_minutos (int): Tiempo máximo en minutos para el procesamiento
    """
    ...
    try:
        ...
        # Convertir Compo_Coste a texto para mejor manejo
        df['Compo_Coste'] = df['Compo_Coste'].astype(str)
        
        # Análisis de componentes de coste
        print("\nComponentes de coste únicos:")
        compo_counts = df['Compo_Coste'].value_counts()
        print(compo_counts.head(10))  # Mostrar los 10 más frecuentes
    ...
    try:
        ...
        df['Descripcion'] = np.where(
            ...
            np.where(
                ...
                df['Prj'] + "." + df['Parte_Prj'] + " - " + df['Compo_Coste']
            ),
    ...
    # PASO 4A: TRANSFORMACIÓN PIVOT PARA COMPONENTES DE COSTE (OPCIONAL)
    if hacer_pivot:
        print("\nRealizando transformación pivot para componentes de coste...")
        try:
            # Determinar los componentes de coste más significativos
            # Agrupar componentes poco frecuentes como "Otros"
            threshold_frecuencia = 10  # Umbral para considerar un componente como frecuente
            compo_counts = df['Compo_Coste'].value_counts()
            compo_principales = compo_counts[compo_counts >= threshold_frecuencia].index.tolist()
            
            # Para los componentes agrupados (01, 03, etc.)
            compo_agrupados = ['01', '03']  # Añadir más según corresponda
            
            # Asegurar que los agrupados estén incluidos independientemente de frecuencia
            for comp in compo_agrupados:
                if comp not in compo_principales:
                    compo_principales.append(comp)
            
            print(f"Se utilizarán {len(compo_principales)} componentes principales para el pivot")
            
            # Crear una copia simplificada para el pivot
            df_pivot = df[['Cia', 'Prj', 'Parte_Prj', 'Nivel', 'Nodo', 'Compo_Coste', 'Valor']].copy()
            
            # Simplificar componentes menos frecuentes
            df_pivot['Compo_Coste'] = np.where(
                df_pivot['Compo_Coste'].isin(compo_principales),
                df_pivot['Compo_Coste'],
                'Otros'
            )
            
            # Realizar pivot - usamos un enfoque de agrupación + pivotado
            pivot_df = df_pivot.pivot_table(
                index=['Cia', 'Prj', 'Parte_Prj', 'Nivel', 'Nodo'],
                columns='Compo_Coste',
                values='Valor',
                aggfunc='sum',
                fill_value=0
            ...
            for col in pivot_df.columns:
                if col not in ['Cia', 'Prj', 'Parte_Prj', 'Nivel', 'Nodo']:
                    renamed_cols[col] = f'Coste_{col}'
            
            pivot_df = pivot_df.rename(columns=renamed_cols)
            
            # Añadir columna de valor total
            pivot_df['Valor_Total'] = pivot_df[[col for col in pivot_df.columns 
                                              if col.startswith('Coste_')]].sum(axis=1)
            
            # Combinar con el DataFrame original para mantener otras columnas
            df = df.drop(columns=['Valor'])  # Eliminamos el valor original para evitar duplicados
            
            # Fusionar basado en las claves
            df = pd.merge(
                df,
                pivot_df,
                on=['Cia', 'Prj', 'Parte_Prj', 'Nivel', 'Nodo'],
                how='left'
            )
            
            # Usar Valor_Total como el nuevo Valor principal
            df['Valor'] = df['Valor_Total']
            
            print("Transformación pivot completada.")
        except Exception as e:
            print(f"Error en transformación pivot: {str(e)}")
            traceback.print_exc()
            print("Continuando sin transformación pivot...")
    
    # PASO 4B: GENERAR COLUMNAS DE NIVEL CON FORMATO SIMPLIFICADO
    ...
    try:
        ...
        if hacer_pivot:
            ...
            # Columnas de coste por componente
            cost_columns = [col for col in result_df.columns if col.startswith('Coste_')]
            
            # Columnas de nivel
            level_columns = [f'Nivel{i}' for i in range(1, nivel_maximo + 1)]
            
            # Combinar todas las columnas
            columns_to_keep = columns_to_keep + cost_columns + level_columns
        else:
            # Sin pivot, incluir Compo_Coste en el resultado
            columns_to_keep = ['Cia', 'Prj', 'Parte_Prj', 'Nivel', 'Compo_Coste', 'Valor'] + [f'Nivel{i}' for i in range(1, nivel_maximo + 1)]
...
def main():
    try:
        ...
        # Procesar datos con transformación pivot
        hacer_pivot = True  # Cambiar a False para desactivar pivot
        
        result_df = procesar_datos_jerarquicos(df, hacer_pivot=hacer_pivot, timeout_minutos=5)
        
        if result_df is not None:
            # Escribir resultados en una nueva hoja
            output_path = excel_path.parent / "Resultado_Jerarquico_Completo.xlsx"
