...
class DataProcessor:
    ...
    def _process_historic_data(self, result, debug=False):
        ...
        if debug:
            ...
            # Mostrar ejemplo de una celda con histórico si existe
            historic_cells = [k for k, v in result['cellData'].items() if v.get('hasHistoric', False)]
            if historic_cells:
                sample_key = historic_cells[0]
                sample_cell = result['cellData'][sample_key]
                print(f"Ejemplo de celda con histórico ({sample_key}):")
                print(f"  ROW: {sample_cell.get('ROW')}, COLUMN: {sample_cell.get('COLUMN')}")
                print(f"  hasHistoric: {sample_cell.get('hasHistoric')}")
                print(f"  Número de puntos en timeSeries: {len(sample_cell.get('timeSeries', []))}")
                if sample_cell.get('timeSeries'):
                    print(f"  Primer punto de la serie: {sample_cell['timeSeries'][0]}")
